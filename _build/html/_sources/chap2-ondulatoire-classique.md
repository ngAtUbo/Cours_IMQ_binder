...
conduisant à l\'<u>intensité [^11] instantanée</u>
...
de l'œil est de 0.1 s et une photodiode de l'ordre de 10^-6^s. La
...
car la moyenne temporelle de $\cos(\omega t + \varphi)$ vaut $½$. On
...
un état excité[^12], peut revenir dans son état fondamental (état de
...
élevée que celle de la lumière visible. **Deux points d\'une même
...
matière lorsqu'il se trouve dans un état excité[^13], peut revenir dans
...
d'extension finie dans le temps) {index}`train d’onde` contenant $(\tau \times c)/\text{λ}$ oscillations.
...
cette aire est l'ordre de $410^{- 2}$mm^2^ raison pour laquelle les
...
[^11]: L\'intensité instantanée électromagnétique est à distinguer de
...
[^12]: Les niveaux d'énergies d'un atome sont quantifiés comme on
    s'attachera à le montrer par la suite.

[^13]: Les niveaux d'énergies d'un atome sont quantifiés comme on
    s'attachera à le montrer par la suite.
